...
public class BasicController {
	...
	public String goA() {
		...
		// return "goA"; 라고 적으면 views폴더내부의 goA.jsp파일을 보여줍니다. 
		return "goA";
	}
	...
	public String getData(String data1, int data2, Model model) { 
		...
		// String data1 = request.getParameter("data"); // jsp때 데이터를 받아오는 방법
		// String date2 = Integer.parseInt(strDate2); // jsp에서 받아온 데이터를 다른 자료형으로 변환하는 방법
		System.out.println("data1에 든 값 : " + data1);
		System.out.println("data2에 든 값 : " + data2);
		System.out.println("data2가 정수임을 증명 : " + (data2+100));
		// data1, data2 변수를 getResult.jsp 보내서 위 콘솔메세지를 화면에서 조회가능하게 해주세요. 
	...
	public String mExchange(int won, Model model) {
		...
		System.out.println("현재 바트와 환율은 36.16원당 1바트 입니다.");
		...
		// won변수에 해당하는변수도 추가로보내주세요. 
		// exchange.jsp를 타겟으로 하니 viesw폴더에 생성해주세요.
		return "exchange";
	}
	...
	// 스프링 5버전부터 허용
	// @요청메서드 Mapping 은 해당 메서드만 허용하는 어노테이션입니다. 
	...
	public String scoreResult(int math, 
			int eng, 
			int lang, 
			int social, 
			// computer -> com 으로 입력하는 로직
	...
	public String getPage(@PathVariable int pageNum, 
		...
		// page.jsp 본문에 현재 ${page} 페이지를 보고계십니다. 
		// 와 함께 입숨더이데이터를 이용해 본문글을 채워주세요. 
		return "page";
	}
	...
	public String getList(
			// 배열자료를 받을시 @RequestParam 사용시 강제됩니다. 
	...
	public String getUserInfo(UserVO userVO, Model model) {
		...
		return "user"; // user.jsp 에서 볼 수 있도록 el문법을 사용해주세요
	}
