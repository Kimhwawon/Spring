...
public interface BoardMapper {
	...
	// insert구문 실행용으로 메서드를 선업합니다.
	// VO내부에 적혀잇는 정보를 이용해 insert를 합니다.
	...
	// delete구문
	// 글 삭제는 DELETE 구문으로 하는데, 비SELECT 구문이르모 리턴자료를 void로 적습니다. 
	// 하나의 글만 삭제할 예정이므로, 삭제할 글 정보를 호출시 같이 입력하게 합니다. 
	public void delete(long bno);
	
	// 글 수정은  UPDATE 구문으로 합니다. 메서드 선언해주세요. 
	// 전달변수가 title, content, bno이므로 , 단일 자료가 아닌 묶음으로 전달합니다 
	public void update(BoardVO vo);
	
	// vo안쓰고 데이터 전달하기
	// 2개이상의 파라미터를 따로따로 전달할때는 각 파라미터 왼쪽에 @Param("보낼 명칭")을 붙여줍니다. 
